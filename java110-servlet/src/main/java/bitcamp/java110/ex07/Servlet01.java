/* 
  ...
  => 보관소
     1) ServletContext 보관소
        - 생성시점 : 웹 애플리케이션이 시작될 때 각 웹 애플리케이션마다 한 개의 보관소가 생성됨
        - 소멸시점 : 웹 애플리케이션이 종료될 때.
        - 저장대상 : 서블릿들이 공유하는 객체를 주로 보관한다.
          예) DAO, 서비스 객체 등
     2) HttpSession 보관소
        - 생성시점 : 각 클라이언트에 대해 세션을 시작할 때 마다.
        - 소멸시점 : 각 클라이언트에 대해 생성된 세션을 종료할 때 마다.
        - 저장대상 : 클라이언트 전용 정보를 보관한다.
          예) 로그인 정보, 페이지 사이에 공유하는 정보.
     3) ServletRequest 보관소
        - 생성시점 : 각 요청이 들어올 때 마다.
        - 소멸시점 : 응답을 완료할 때 마다.
        - 저장대상 : 한 요청을 처리하는 서블릿들이 공유하는 정보.
     4) PageContext 보관소
        - 생성시점 : JSP가 실행될 때 마다.
        - 소멸시점 : JSP 실행을 마쳤을 때 마다.
        - 저장대상 : JSP와 태그 핸들러가 공유하는 정보
 */
...
public class Servlet01 extends HttpServlet {
    ...
    public void service(
        ...
        out.printf("ServletContect : aaa=%s \n", sc.getAttribute("aaa"));
