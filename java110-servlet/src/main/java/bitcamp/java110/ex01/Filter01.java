...
// 필터를 만들었으면 어떤 요청에 대해 실행할 것 인지 등록해야한다.
// URL은 반드시 "/"로 시작해야 한다.
//@WebFilter({"/ex01/*","/ex02"})
public class Filter01 implements Filter {
    ...
    public void doFilter(
        ...
        // 클라이언트가 요청한 서블릿을 실행하기 전에 해당 URL에 등록한 필터를 실행한다. 이때 이 메서드가 호출된다.
        ...
        // 위 메서드를 호출하지 않으면 다음 필터나 서블릿을 실행하지 않는다. 즉, 빼면 안된다!!
    ...
    @Override
    public void destroy() {
        // 톰캣 서버가 종료되거나 웹 애플리케이션이 멈출 때 생성된 모든 필터에 대해 서블릿 컨테이너가 이 메서드를 호출한다.
