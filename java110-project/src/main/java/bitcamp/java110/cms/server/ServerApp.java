    ...
    class RequestWorker implements Runnable {
        ...
        public void run() {
            ...
            try (
                ...
                try {
                    // 요청 핸들러 호출
                    mapping.getMethod().invoke(mapping.getInstance(), request, response); // static메소드를 호출할땐 첫번재매개변수에
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        private void responseHTTPMessage(PrintWriter out, String message) {
            ...
            out.println("Content-type: text/plain;charset=UTF-8"); // client에게 보내는 텍스트는 순수한 텍스트다라는걸 알려주는부분.
            out.println();
            out.print(message);
            out.flush();
        }
