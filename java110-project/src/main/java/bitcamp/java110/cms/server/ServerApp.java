...
// 이 애플리케이션의 특정 URL에 대해 요청이 들어왔을 때 톰캣 서버가 이 서블릿을 실행하도록 등록한다.
...
public class ServerApp implements Servlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        // 서블릿 컨테이너는 서블릿 객체를 생성한 후, 이 서블릿이 실행하는데 필요한 자원을 준비할 수 있도록 딱 한번 호출한다.
    ...
    public ServletConfig getServletConfig() {
        /* 서블릿에서 작업을 하는 동안, 서블릿 설정 정보를 참조할 필요가 있을 때 이 메서드를 호출한다.
                         ...
                         보통 init()가 호출될 때 받은 파라미터 값을 그대로 리턴한다.*/
        return this.config;
    }
    ...
    public void service(ServletRequest request, ServletResponse response) throws ServletException, IOException {
        ...
        // => HTTP 프로토콜 정보를 다루려면 request 객체를 원래 타입으로 캐스팅 한 후 사용해라!
    ...
    public String getServletInfo() {
        ...
        return "클라이언트 요청을 중계하는 서블릿";//실제로 요청을 실행하는건 Controller임
    }

    @Override
    public void destroy() {
        /* 서블릿 컨테이너는, 서버를 종료하거나 웹 애플리케이션을 정지하기 바로 직전에 이 메서드를 호출한다.
         * 그 이유는, 이 서블릿이 사용했던 자원을 해제시켜 메모리를 줄일 수 있도록 하기 위함이다.
         * 예를들어 DB연결 된 것을 닫을 때, 열린 파일을 닫을 때, 연결된 소켓을 닫을 때 등의 작업을 이 메소드에서 수행하면 된다.
         */
    }
}// ServletApp class