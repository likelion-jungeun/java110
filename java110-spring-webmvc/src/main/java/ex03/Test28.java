...
public class Test28 {
    ...
    @ResponseBody
    public String m2(
            // @SessionAttributes에 지정된 값을 받고싶을 때
    ...
    public String logout2(SessionStatus status) throws Exception {
        ...
        status.setComplete();
        return String.format("@SessionAttributes관리 대상을 제거했다.\n");
    }
}


// HttpSession vs @SessionAttributes
// => 세션에 값을 저장할 때, 모든 페이지 컨트롤러에서 사용할 값이라면 HttpSession 객체에 직접 저장하라!
//    특정 페이지 컨트롤러에서만 사용하고 관리될 값이라면 @SessionAttributes로 등록하라!
// 
// => 값 제거
//    HttpSession.invalidate()는 세션을 완전히 무효화 시킨다.
//    SessionStatus.setComplete()는 해당 페이지 컨트롤러의
//    @SessionAttribute에 지정된 값만 세션 객체에서 제거한다.
//    setComplete()는 세션을 무효화시키지 않는다.
