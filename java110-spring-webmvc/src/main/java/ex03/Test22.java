...
public class Test22 {
    ...
    // 프론트 컨트롤러는 request handler를 실행한 후 리턴 값을 클라이언트로 바로 출력한다.
    ...
    // 출력하는 데이터가 application/json 이라고 프론트 컨트롤러에게 알려주면,
    ...
    public Car m1() throws Exception {
        ...
        car.setMusics(new String[]{"오호라!","우헤헤","시간이 지나후"});
    ...
    // produces 속성에 어떤 콘텐트를 리턴할지 그 타입을 지정하지 않는다면,
    ...
    // @RestController일 경우, 리턴 타입이 primitive type 이라면 
    // 문자열로 변환하여 그대로 출력한다.
    @RequestMapping("m5")
